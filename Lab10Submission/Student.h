...
public:
    ...
    /// \brief Checks if M Number of left is less than or equal to right
    /// \returns Whether the left side's M Number is less than or equal
    bool operator<=(const Student& rhs) const;

    /// \brief Checks if M Number of left is greater than or equal to right
    /// \returns Whether the left side's M Number is greater than or equal
    bool operator>=(const Student& rhs) const;
