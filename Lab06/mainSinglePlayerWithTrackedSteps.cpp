...
int main() {
    ...
    Queue<Move>* movesTaken = new Queue<Move>();
    ...
    while (!done(stacks)) {
        try {
            ...
            // record the move taken
            movesTaken->enqueue({selections[0], selections[1]});
            printStacks(stacks);
        } catch (...) {
...
void printMoves(Queue<Move>* moves) {
    cout << endl << "Moves taken to win:" << endl << endl;
    int step = 1;
    while(!moves->isEmpty()) {
        Move move = moves->dequeue();
        cout << "Move " << step << ": " <<
                "From stack " << move.from << " to stack " << move.to << endl;
        step++;
    }
}